...
const state = {session:null, inventory:null, config:null, operations:[], plan:null, applyKey:null, page:'serving', busy:false, memory:[], memoryError:'', memorySelection:null, memoryPreview:null, memoryPreviewError:'', memoryAdvice:null};
...
function showLogin(){state.session=null;state.memory=[];state.memorySelection=null;state.memoryPreview=null;state.memoryAdvice=null;$('login').hidden=false;$('management').hidden=true;$('logout').hidden=true;$('identity').textContent='';}
...
function memorySummary(summary){
...
function renderMemoryAdvice(){return state.memoryAdvice?`<h3>Unapproved advisory explanation</h3><pre>${escape(state.memoryAdvice.summary)}</pre><p class="warning">The explanation is advisory text. Review the separately validated plan before confirming any export.</p>`:'';}
...
function operationTable(ops) {
  ...
  return `<div class="table-wrap"><table><thead><tr><th>Operation / action</th><th>Progress</th><th>Durable outcome</th><th>Actions</th></tr></thead><tbody>${ops.map(o=>`<tr>
    ...
    ${(o.artifacts||[]).map((a,i)=>a.content||(memoryOwner()&&o.plan.draft.action==='memory.plan.export'&&privateMemoryArtifact(a.name))?`<div class="actions"><button data-artifact="${escape(o.id)}" data-index="${i}">Download ${escape(a.name)}</button></div>`:'').join('')}</td>
